...
<div class="bootstrap-admin-panel-content">
   @if(Picture::all()->count() == 0)
    <h3>There are no Photo posted</h3>
    @else
    ...
    <tbody>
    <?php $i=1; ?>
    @foreach(Picture::all() as $us)
    <tr>
        <td>{{ $i++ }}</td>
        <td><img src='{{ asset("uploads/pictures/{$us->image}") }}' style="width: 70px; height: 70px"/></td>
        <td>{{ $us->description }}</td>
        <td >{{ $us->category->tittle }}</td>
        <td id="{{ $us->id }}">
...
<script>
    ...
    $(document).ready(function() {
        $('#example2').dataTable({
            "fnDrawCallback": function( oSettings ) {
                ...
                $(".edituser").click(function(){
                    ...
                    modal+= '<h2 class="modal-title" id="myModalLabel">Update Photo Information</h2>';
                    modal+= '</div>';
                    ...
                    $(".modal-body").load("<?php echo url("admin/picture/edit") ?>/"+id1);
                ...
                $(".deletevaccine").click(function(){
                    ...
                    $("#yes").click(function(){
                        ...
                        $.post("<?php echo url('admin/picture/delete') ?>/"+id1,function(data){
        ...
        $(".add").click(function(){
            ...
            modal+= '<div class="modal-header">';
            modal+= '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>';
            modal+= '<h2 class="modal-title" id="myModalLabel">Add New Photo</h2>';
            modal+= '</div>';
            modal+= '<div class="modal-body">';
            modal+= ' </div>';
            modal+= '</div>';
            ...
            $("#myModal").modal("show");
            $(".modal-body").html("<h3><i class='fa fa-spin fa-spinner '></i><span>loading...</span><h3>");
            $(".modal-body").load("<?php echo url("admin/picture/add/") ?>");
            $("#myModal").on('hidden.bs.modal',function(){
                $("#myModal").remove();
            })
